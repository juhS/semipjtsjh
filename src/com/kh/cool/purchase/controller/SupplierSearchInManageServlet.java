...
/**
 * Servlet implementation class SupplierSearchInManageServlet
 */
@WebServlet("/searchSupinManage.pu")
public class SupplierSearchInManageServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SupplierSearchInManageServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(searchCondition.equals("거래처명")) {
			list = new PurchaseService().selectSupplierWithSupName(searchValue);
			//System.out.println("list 확인 in servlet" + list);
			
		//검색조건 거래처 코드일 때
		} else {
			list = new PurchaseService().selectSupplierSupCode(searchValue);
		}
